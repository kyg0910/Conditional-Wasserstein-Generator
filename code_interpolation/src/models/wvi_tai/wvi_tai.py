...
class WVITAIFillInModel(nn.Module):
    """A video frame inpainting network that predicts two sets of middle frames and then blends them with the WVI_TAI
    module.

    WVI_TAI computes the kernels that get applied to the intermediate predictions using the intermediate
    ...
    def __init__(self, gf_dim, c_dim, feature_size, ks, num_block=5, kf_dim=32, layers=3, forget_bias=1,
                 activation=F.tanh, bias=True, gpu_id = [0]):
        """Constructor
        ...
        :param gpu_id: The index of gpu
        """

        super(WVITAIFillInModel, self).__init__()

        self.c_dim = c_dim
        self.conv_lstm_state_size = 8 * gf_dim

        self.generator = torch.nn.DataParallel(MCNet(gf_dim, c_dim, feature_size, forget_bias=forget_bias, activation=activation, bias=bias, is_stochastic=True), gpu_id)

        self.merge_residual3 = torch.nn.DataParallel(Residual(gf_dim * 8, kf_dim * 4), gpu_id)
        self.merge_residual2 = torch.nn.DataParallel(Residual(gf_dim * 4, kf_dim * 2), gpu_id)
        self.merge_residual1 = torch.nn.DataParallel(Residual(gf_dim * 2, kf_dim * 1), gpu_id)

        self.kernelnet = torch.nn.DataParallel(TAI(gf_dim, ks, num_block, layers, kf_dim), gpu_id)
    ...
    def forward_train(self, T, preceding_frames, gt_middle_frames, following_frames, gamma):
        ...
        # Compute forward difference frames
        preceding_middle_frames = torch.cat([preceding_frames, gt_middle_frames], dim = 1)
        gray_imgs_t_preceding_middle = bgr2gray_batched(inverse_transform(preceding_middle_frames)) if preceding_middle_frames.size(2) > 1 else inverse_transform(preceding_middle_frames)
        diff_in = gray_imgs_t_preceding_middle[:, 1:, :, :, :] - gray_imgs_t_preceding_middle[:, :-1, :, :, :]
        
        # Compute backward difference frames
        middle_following_frames = torch.cat([gt_middle_frames, following_frames], dim = 1)
        gray_imgs_t_middle_following = bgr2gray_batched(inverse_transform(middle_following_frames)) if middle_following_frames.size(2) > 1 else inverse_transform(middle_following_frames)
        rev_indexes = Variable(torch.LongTensor(range(T+F-1, -1, -1))).cuda()
        rev = gray_imgs_t_middle_following.index_select(1, rev_indexes)
        diff_in_F = rev[:, 1:, :, :, :] - rev[:, :-1, :, :, :]

        # Generate the forward and backward predictions
        forward_pred_post, forward_dyn_post, forward_cont_post, forward_res_post, forward_dyn_prior, forward_h_cond = self.generator(K, T, diff_in, xt, gt_middle_frames, is_train=True)
        backward_pred_post, backward_dyn_post, backward_cont_post, backward_res_post, backward_dyn_prior, backward_h_cond = self.generator(F, T, diff_in_F, xt_F, gt_middle_frames.index_select(1, Variable(torch.LongTensor(range(T-1, -1, -1))).cuda()), is_train=True)
        
        # Correct the order of the backward frames
        backward_pred_post = backward_pred_post[::-1]
        backward_dyn_post = backward_dyn_post[::-1]
        backward_cont_post = backward_cont_post[::-1]
        backward_res_post = backward_res_post[::-1]
        backward_dyn_post = backward_dyn_post[::-1]
        backward_h_cond = backward_h_cond[::-1]

        # Store the final predictions and pre-summed outputs of the interpolation network
        combination_post = []
        interp_net_outputs_1_post = []
        interp_net_outputs_2_post = []
        # Compute the time information to inject
        w = np.linspace(0, 1, num=T+2).tolist()[1:-1]
        for t in range(T):
            merged_res_post = []
            merged_res_post.append(self.merge_residual1(forward_res_post[t][0], backward_res_post[t][0]))
            merged_res_post.append(self.merge_residual2(forward_res_post[t][1], backward_res_post[t][1]))
            merged_res_post.append(self.merge_residual3(forward_res_post[t][2], backward_res_post[t][2]))
            # Do time-aware frame interpolation on the forward and backward predictions
            variableDot1_post, variableDot2_post = self.kernelnet(forward_pred_post[t], 
                                                                  backward_pred_post[t],
                                                        forward_dyn_post[t], backward_dyn_post[t],
                                                        forward_cont_post[t], backward_cont_post[t],
                                                        merged_res_post, ratio=1-w[t])

            # Store the pre-summed outputs of the interpolation network
            interp_net_outputs_1_post.append(variableDot1_post)
            interp_net_outputs_2_post.append(variableDot2_post)
            # Merge the modified forward and backward predictions
            combination_post.append(0.5 * variableDot1_post + 0.5 * variableDot2_post)
        
            # Store the dyn outputs
            dyn_post_forward, dyn_prior_forward = forward_dyn_post[t], forward_dyn_prior[t]
            dyn_post_backward, dyn_prior_backward = backward_dyn_post[t], backward_dyn_prior[t]
            h_cond_forward, h_cond_backward = forward_h_cond[t], backward_h_cond[t]
            
            if t == 0:
                dyn_post_forward_seq, dyn_prior_forward_seq = dyn_post_forward, dyn_prior_forward
                dyn_post_backward_seq, dyn_prior_backward_seq = dyn_post_backward, dyn_prior_backward
                h_cond_forward_seq, h_cond_backward_seq = h_cond_forward, h_cond_backward
            else:
                dyn_post_forward_seq = torch.cat([dyn_post_forward_seq, dyn_post_forward], dim=1)
                dyn_prior_forward_seq = torch.cat([dyn_prior_forward_seq, dyn_prior_forward], dim=1)
                dyn_post_backward_seq = torch.cat([dyn_post_backward_seq, dyn_post_backward], dim=1)
                dyn_prior_backward_seq = torch.cat([dyn_prior_backward_seq, dyn_prior_backward], dim=1)
                h_cond_forward_seq = torch.cat([h_cond_forward_seq, h_cond_forward], dim=1)
                h_cond_backward_seq = torch.cat([h_cond_backward_seq, h_cond_backward], dim=1)
        
        dyn_prior_seq = torch.cat([dyn_prior_forward_seq, dyn_prior_backward_seq], dim=1)
        dyn_post_seq = torch.cat([dyn_post_forward_seq, dyn_post_backward_seq], dim=1)
        h_cond_seq = torch.cat([h_cond_forward_seq, h_cond_backward_seq], dim=1)
        
        if gamma == 0.0:
            shuffled_idx, combination_post_shuffled = None, None
        else:
            # Compute the shuffled time information to inject
            shuffled_idx = np.arange(preceding_frames.size(0))
            np.random.shuffle(shuffled_idx)

            combination_post_shuffled = []        
            for t in range(T):            
                merged_res_post_shuffled = []
                merged_res_post_shuffled.append(self.merge_residual1(forward_res_post[t][0][shuffled_idx],
                                                            backward_res_post[t][0][shuffled_idx]))
                merged_res_post_shuffled.append(self.merge_residual2(forward_res_post[t][1][shuffled_idx],
                                                            backward_res_post[t][1][shuffled_idx]))
                merged_res_post_shuffled.append(self.merge_residual3(forward_res_post[t][2][shuffled_idx],
                                                            backward_res_post[t][2][shuffled_idx]))

                # Do time-aware frame interpolation on the forward and backward predictions
                variableDot1_post_shuffled, variableDot2_post_shuffled = self.kernelnet(forward_pred_post[t][shuffled_idx],
                                                                                        backward_pred_post[t][shuffled_idx],
                                                                                        forward_dyn_post[t].detach(),
                                                                                        backward_dyn_post[t].detach(),
                                                                                        forward_cont_post[t][shuffled_idx],
                                                                                        backward_cont_post[t][shuffled_idx],
                                                                                        merged_res_post_shuffled,
                                                                                        ratio=1-w[t])

                # Merge the modified forward and backward predictions
                combination_post_shuffled.append(0.5 * variableDot1_post_shuffled + 0.5 * variableDot2_post_shuffled)
            combination_post_shuffled = torch.stack(combination_post_shuffled, dim=1)
        
        # Stack time dimension (T x [B x C x H x W] -> [B x T x C x H x W]
        combination_post = torch.stack(combination_post, dim=1)
        forward_pred_post = torch.stack(forward_pred_post, dim=1)
        backward_pred_post = torch.stack(backward_pred_post, dim=1)
        interp_net_outputs_1_post = torch.stack(interp_net_outputs_1_post, dim=1)
        interp_net_outputs_2_post = torch.stack(interp_net_outputs_2_post, dim=1)
        
        return {
            'pred': combination_post,
            'pred_forward': forward_pred_post,
            'pred_backward': backward_pred_post,
            'interp_net_outputs_1': interp_net_outputs_1_post,
            'interp_net_outputs_2': interp_net_outputs_2_post,
            'z_prior_seq': dyn_prior_seq,
            'z_post_seq': dyn_post_seq,
            'h_cond_seq': h_cond_seq,
            'pred_shuffled': combination_post_shuffled,
