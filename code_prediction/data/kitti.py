...
class KITTI(object):
    ...
    """Data Handler that loads KITTI data."""

    def __init__(self, data_root, type='train', seq_len=20, image_size=64):
        self.root_dir = data_root 
        if type=='train':
            self.data_dir = '%s/train' % self.root_dir
            self.ordered = False
        elif type=='val':
            self.data_dir = '%s/val' % self.root_dir
            self.ordered = True 
        elif type=='test':
    ...
    def get_seq(self):
        ...
        image_seq = []
        st = np.random.randint(len(os.listdir(d))-self.seq_len + 1) #randomly sample starting index from video
        for i in range(self.seq_len):
            fname = f'{d}/{st+i:010d}.png'
